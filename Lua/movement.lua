...
This code is a combination of the original movement.lua by Hempuli and
movement.lua from Patashu's redux mods. Some of it was also written
			...
			for i,data in ipairs(moving_units) do
				...
				if (data.state == state) and (data.moves > 0) and (data.unitid ~= 0) then
					...
					if (x ~= -1) and (y ~= -1) and (holder == 0) then
                        ...
                        local id = mmf.newObject(data.unitid).values[ID]

						if (encased_ids[id]) then
							units = get_encased_units(data.unitid);
                            pushers = units
							for _, pusher in ipairs(units) do
								...
								merge_table(obslist, obslist_);
								merge_table(allobs, allobs_);
								merge_table(specials, specials_);
			...
			if (id == -1) then
				...
				if valid then
					...
					if (localresult ~= 1) and (localresult ~= -1) then
						...
						if (ispull ~= nil) and pulling and not is_unit_encased(id) then
							table.insert(result, id)
							table.insert(results, id)
							added = true
						end
...
function trypush(unitid,ox,oy,dir,pulling_,x_,y_,reason,pusherid,is_encase)
	local x,y = 0,0
	local unit = {}
	local name = ""
    ...
    -- Encased objects are moved together. The whole thing has to move or nothing moves.
	if (not is_encase and is_unit_encased(unitid)) then
		local units = get_encased_units(unitid);
				...
				while (done == false) do
					...
					elseif (hm == 1) or (hm == -1) then
						...
						if (pulling == false) then
							hm = trypush(hm,ox,oy,dir,pulling,x+ox,y+oy,reason,unitid,is_encase)
						else
							result = math.max(0, result)
							done = true
						end
...
function dopush(unitid,ox,oy,dir,pulling_,x_,y_,reason,pusherid,is_encase)
    ...
    -- Encased objects move together
    -- At this point, we know it will succeed
	if (not is_encase and is_unit_encased(unitid) and not pulling) then
		local units = get_encased_units(unitid)
        local positions = {}

        -- Only push if the object it will push onto is not encased.

        for _, u in ipairs(units) do
            local obj = mmf.newObject(u)
            positions[obj.values[XPOS] .. "," .. obj.values[YPOS]] = true
        end

        for _, u in ipairs(units) do
            local obj = mmf.newObject(u)
            if (not positions[(obj.values[XPOS] + ox) .. "," .. (obj.values[YPOS] + oy)]) then
                dopush(u,ox,oy,dir,pulling_,x_,y_,reason,pusherid,true)
            else
...
function find_encased_pulls(unitid,dir)
	local result = {}
	...
	for _, id in ipairs(obs) do
		if (id ~= -1) then
			local obsunit = mmf.newObject(id)
			local obsname = getname(obsunit)
			if hasfeature(obsname, "is", "pull", id) and
                (isstill_or_locked(id, curx, cury, dir) == false) and
                is_unit_encased(id)
            then
				table.insert(result, id);
			end
...
function queue_move(unitid,ox,oy,dir,specials,reason,x,y)
	...
	if (unitid ~= 2) then
		local others = find_encased_pulls(unitid, dir);
...
function get_encased_units(unitid)
    local result = {}
    if (encased_ids[mmf.newObject(unitid).values[ID]]) then
        for id, _ in pairs(encased_ids) do
            result[#result + 1] = MF_getfixed(id)
        end
    end
    return result
end

function merge_table(t, other)
	if other ~= nil then
...
function is_unit_encased(unitid)
    return encased_ids[mmf.newObject(unitid).values[ID]]
end
